...
export const NavControllerOriginal: React.FC<NavControllerProps> = ({
  pressedButtons,
  configs,
  showDiagonals,
  showActions,
  ...
  const renderArrowBtn = (id: ButtonId, rotate: number) => {
    ...
    return (
      <button
        ...
        onPointerDown={(e) => { e.preventDefault(); onButtonDown(id); }}
        onPointerUp={(e) => { e.preventDefault(); onButtonUp(id); }}
        onPointerLeave={(e) => { e.preventDefault(); onButtonUp(id); }}
  ...
  return (
    <div 
      className="relative w-auto h-auto flex flex-col items-center justify-center no-context-menu"
    ...
    >
      ...
      */}
      <div 
        className={`relative bg-white border border-gray-200 p-10 rounded-[2.5rem] shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] flex items-center transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${!showActions ? 'aspect-square justify-center' : 'pr-12'}`}
      >
