...
export function MasterRompler({ isOpen, onToggle, sabBuffer }) {
  ...
  const handleMidiChanMouseDown = (e) => {
    e.preventDefault();
  ...
  return (
    <div className="shrink-0 relative master-rompler-wrapper text-[#c5c6c7] flex flex-col overflow-visible">

      {/* Toggle Tab — sits above the drawer, flush left */}
      <div
        onClick={onToggle}
        className="absolute bottom-full left-0 h-7 px-4 bg-[#121213] text-white rounded-tr-lg cursor-pointer border-t border-r border-[#1e1e20] shadow-[0_-6px_16px_rgba(0,0,0,0.4)] flex items-center gap-2 hover:bg-[#1a1a1c] transition-colors z-30"
      >
      ...
      {/* Drawer Body — animated open/close */}
      <motion.div
        initial={false}
        ...
        className={cn(
          ...
          isOpen 
            ? "bg-[#121213] border-t border-[#1e1e20] shadow-[0_-12px_30px_rgba(0,0,0,0.6)]" 
        ...
        <div className="max-w-[900px] w-full flex flex-col items-stretch">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-[#242426]">
            <div className="flex items-baseline gap-2">
              <h2 className="text-white font-black italic tracking-tighter text-xl m-0">
                MIDI ROMPLER
              </h2>
            ...
            <div className="w-full bg-[#242426] rounded-lg shadow-[0_10px_20px_rgba(0,0,0,0.5),inset_0_2px_4px_rgba(255,255,255,0.05)] border border-[#333] relative flex px-6 pt-6 pb-6">
              ...
              <div className="w-full flex items-start justify-between z-10 gap-6">
                ...
                <div className="flex flex-col flex-shrink-0">
                  ...
                  >
                    <span className="text-[#00ff88] digital-led text-xl tracking-wide drop-shadow-[0_0_8px_rgba(0,255,136,0.6)]">
                      {String(midiChannel).padStart(2, '0')}
                    </span>
                  </div>
                  <div className="flex justify-between w-14">
                    <button
                      className="w-[26px] h-6 rounded border border-[#111] bg-[#1a1a1c] text-[#8a8a93] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),_0_2px_4px_rgba(0,0,0,0.3)] flex items-center justify-center hover:bg-[#222225] active:scale-95 transition-all outline-none text-xs"
                      onClick={() => setMidiChannel(Math.min(16, midiChannel + 1))}
                    >+</button>
                    <button
                      className="w-[26px] h-6 rounded border border-[#111] bg-[#1a1a1c] text-[#8a8a93] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),_0_2px_4px_rgba(0,0,0,0.3)] flex items-center justify-center hover:bg-[#222225] active:scale-95 transition-all outline-none text-xs"
                      onClick={() => setMidiChannel(Math.max(1, midiChannel - 1))}
                    >-</button>
                  </div>
                </div>
                  ...
                  <div className="relative w-full h-10 bg-[#0a0f0a] rounded border border-[#1a1c1a] shadow-[inset_0_4px_10px_rgba(0,0,0,0.6)] flex items-center px-4 group outline-none overflow-hidden hover:bg-[#0d140d] transition-colors">
                    ...
                    {isLoading ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[#00ff88] digital-led text-xs tracking-widest opacity-80">LOADING...</span>
                      </div>
                    ) : (
                      <span className="text-[#00ff88] digital-led text-sm tracking-[0.1em] drop-shadow-[0_0_8px_rgba(0,255,136,0.6)] uppercase select-none w-full flex justify-between items-center">
                        {instrument.replace('-', ' ')}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
          <div className="bg-[#121213] border border-[#2a2a2e] p-8 rounded-lg max-w-sm w-full relative shadow-2xl">
            ...
            <div className="space-y-4 text-sm text-[#8a8a93] leading-relaxed font-sans">
              ...
              <div className="w-full h-px bg-[#2a2a2e] my-6" />
              <div className="mt-6">
                ...
                <ul className="space-y-3 text-xs text-[#8a8a93] leading-relaxed">
                  <li><strong className="text-[#00ff88]">Tone.js Instruments:</strong> Acoustic Piano, Guitar, and Bass samples provided by <a href="https://github.com/nbrosowsky/tonejs-instruments" target="_blank" rel="noopener noreferrer" className="text-white hover:underline">nbrosowsky</a>.</li>
                  <li><strong className="text-[#00ff88]">smplr Library:</strong> Electric Piano, Harp, Vibraphone, and Celeste playback powered by <a href="https://github.com/danigb/smplr" target="_blank" rel="noopener noreferrer" className="text-white hover:underline">smplr (danigb)</a>.</li>
                  <li><strong className="text-[#00ff88]">MIDI.js Soundfonts:</strong> String Ensemble sample from MusyngKite, hosted by <a href="https://github.com/gleitz/midi-js-soundfonts" target="_blank" rel="noopener noreferrer" className="text-white hover:underline">gleitz</a>.</li>
                </ul>
              </div>
            </div>
            <button
